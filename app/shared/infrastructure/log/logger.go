...
func setup() *zap.Logger {
	...
	log, _ := cfg.Build()
	log = log.
		With(zap.String("version", os.Getenv("VERSION"))).
		With(zap.String("system", os.Getenv("SYSTEM"))).
		With(zap.String("environment", os.Getenv("ENVIRONMENT"))).
		With(zap.String("country", os.Getenv("COUNTRY"))).
		With(zap.String("service", os.Getenv("SERVICE"))).
		With(zap.String("cloud_provider", os.Getenv("CLOUD_PROVIDER")))
...
func (logger Log) WithFields(fields Fields) *Log {
	...
	for k, v := range fields {
		logCustom.log = logCustom.log.With(zap.Any(k, v))
	}
	return logCustom
}
...
func Printf(message string, args ...interface{}) { instance.Printf(message, args...) }
